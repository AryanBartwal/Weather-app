...
const BackgroundEffects = () => {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      {/* Simplified cyber grid for mobile */}
      {!shouldReduceAnimations && (
      ...
      <div className={`absolute top-0 left-0 w-full h-1/2 bg-gradient-radial from-cyber-500/10 to-transparent ${isMobile ? 'opacity-20' : 'opacity-40'}`}></div>
      <div className={`absolute bottom-0 right-0 w-full h-1/2 bg-gradient-radial from-azure-500/10 to-transparent ${isMobile ? 'opacity-15' : 'opacity-30'}`}></div>
      
      {/* Ambient neon lights - conditional rendering */}
      {!shouldReduceBlur && (
        <>
          <div className={`absolute top-20 right-20 ${isMobile ? 'w-48 h-48' : 'w-96 h-96'} bg-cyber-500/${isMobile ? '5' : '10'} rounded-full ${shouldReduceBlur ? 'blur-xl' : 'blur-3xl'} ${shouldReduceAnimations ? 'opacity-60' : 'animate-pulse-glow'}`}></div>
          <div className={`absolute bottom-40 left-20 ${isMobile ? 'w-40 h-40' : 'w-80 h-80'} bg-azure-500/${isMobile ? '5' : '10'} rounded-full ${shouldReduceBlur ? 'blur-lg' : 'blur-3xl'} ${shouldReduceAnimations ? 'opacity-40' : 'animate-pulse-glow'}`} style={{ animationDelay: shouldReduceAnimations ? 'none' : '1.5s' }}></div>
        </>
      )}
      
      {/* Digital horizon line */}
      <div className={`absolute bottom-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-cyber-500/50 to-transparent opacity-70 ${shouldReduceAnimations ? '' : 'animate-pulse-glow'}`}></div>
      ...
      {/* Floating orbs - only on desktop with good performance */}
      {!shouldReduceAnimations && !shouldReduceBlur && (
        <>
      ...
      {!shouldReduceParticles && (
        <>
          <div className={`absolute top-1/3 right-1/3 w-10 h-10 bg-cyber-400/20 rounded-full blur-md shadow-glow ${shouldReduceAnimations ? 'opacity-60' : 'animate-pulse-glow'}`}></div>
          <div className={`absolute top-2/3 left-1/5 w-8 h-8 bg-cyber-300/20 rounded-full blur-md shadow-glow ${shouldReduceAnimations ? 'opacity-60' : 'animate-pulse-slow'}`} style={{ animationDelay: shouldReduceAnimations ? 'none' : '1.5s' }}></div>
          <div className={`absolute bottom-1/4 right-1/4 w-6 h-6 bg-azure-400/20 rounded-full blur-md shadow-glow ${shouldReduceAnimations ? 'opacity-60' : 'animate-pulse-glow'}`} style={{ animationDelay: shouldReduceAnimations ? 'none' : '3s' }}></div>
        </>
      )}
      
      {/* Tiny glowing particles - adaptive count */}
      {Array.from({ length: shouldReduceParticles ? 5 : 15 }).map((_, index) => (
        <div 
          key={index}
          className={`absolute w-1.5 h-1.5 bg-cyber-400/40 rounded-full shadow-glow ${shouldReduceAnimations ? 'opacity-40' : 'animate-pulse-slow'}`}
          style={{
            top: `${Math.random() * 100}%`,
            left: `${Math.random() * 100}%`,
            filter: 'blur(1px)',
            animationDelay: shouldReduceAnimations ? 'none' : `${Math.random() * 5}s`,
            animationDuration: shouldReduceAnimations ? 'none' : `${3 + Math.random() * 7}s`
          }}
        ></div>
      ))}
      
      {/* Digital haze - simplified for mobile */}
